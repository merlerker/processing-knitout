...
class KnitoutWriter {
    ...
    castOn() {
        ...
        this.output.print(";; END CASTON");
    }
    
    
    // bind off
    bindOff() {
        this.output.print(";; BEGIN BINDOFF");

        if (this.mode == 1) {
            // we are on an even row, so knit left
            if (this.grid_str.length%2 == 0) {
                // both knit left, first yarn 2 (blue) then yarn 1 (red)
                for (let c=this.num_cols-1; c>=0; c--) {
                    this.output.print("knit - f" + str(c) + " " + str(this.carrier2) + "");
                }
            ...
            // we are on an odd row, so knit right then left
            else {
                // both knit right, first yarn 2 (blue) on front then yarn 1 (red) on back
                for (let c=0; c<this.num_cols; c++) {
                    this.output.print("knit + f" + str(c) + " " + str(this.carrier2) + "");
                }
                for (let c=0; c<this.num_cols; c++) {
            ...
            // now both knit right, alternating front and back
            // yarn 1 is on top, so start with yarn 2 so they interlock
            for (let c=0; c<this.num_cols; c++) {
                // even stitches on the front
                if (c%2 == 0) {
            ...
            // knit yarn 1 right on front
            // knit yarn 2 right on back
            for (let c=0; c<this.num_cols; c++) {
                this.output.print("knit + f" + str(c) + " " + str(this.carrier1) + "");
            }
            for (let c=0; c<this.num_cols; c++) {
            ...
            // knit yarn 1 left on back
            // knit yarn 2 left on front
            for (let c=this.num_cols-1; c>=0; c--) {
                this.output.print("knit - b" + str(c) + " " + str(this.carrier1) + "");
            }
            for (let c=this.num_cols-1; c>=0; c--) {
        ...
        this.output.print(";; BINDOFF COMPLETE")
    }
